...
SELECT RIGHT(u.Email, LEN(Email) - CHARINDEX('@', u.Email, 1)) AS [Email Provider], 
	   COUNT(*) AS [Number Of Users] 
  FROM Users AS u
GROUP BY RIGHT(u.Email, LEN(Email) - CHARINDEX('@', u.Email, 1))
...
GROUP BY i.Name, i.Price, i.MinLevel, s.Strength, s.Defence, s.Speed, s.Luck, s.Mind, s.Id
HAVING Mind > (SELECT AVG(Mind) FROM [Statistics]) AND
...
JOIN Continents AS cont ON c.ContinentCode = cont.ContinentCode
GROUP BY c.CountryName, cont.ContinentName, cr.CountryCode
ORDER BY RiversCount DESC, TotalLength DESC, c.CountryName

--11. Count of Countries by Currency
SELECT curr.CurrencyCode, curr.[Description] AS Currency, COUNT(c.CountryCode) AS NumberOfCountries FROM Countries AS c
RIGHT JOIN Currencies AS curr ON c.CurrencyCode = curr.CurrencyCode
GROUP BY curr.CurrencyCode, curr.CurrencyCode, curr.[Description]
